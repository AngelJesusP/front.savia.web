...
const TableGeneric: FC<any> = ({ idEnfermedad, claveArchivo }) => {
  ...
  const getDescriptionError = async (dataTable: any) => {
    ...
    if (elemento != null) {
      ...
      if (valor === "Error_Estructura") {
        elemento.innerHTML =
          "<b>INFORMACIÓN</b><br/>Este registro no cumple con la estructura de los campos deseada.";
        return;
      }
    }
    const json = {
  ...
  const getData = async (values: any) => {
    ...
    if (localStorage.getItem("headers") != null) {
      const jsonHeaders: { data: []; id: number } = JSON.parse(
        localStorage.getItem("headers") as string
      );
    ...
    if (!headers) {
      ...
      localStorage.setItem(
        "headers",
        JSON.stringify({ data: valuesResponse?.lst, id: idEnfermedad })
      );
    } else {
      const jsonHeaders: { data: []; id: number } = JSON.parse(
        localStorage.getItem("headers") as string
